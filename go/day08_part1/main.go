...
func solve(junction_boxes []JunctionBox, connections int) int {
	...
	for i := range connections {
		jb_distance := distances[i]
		union_find.union(jb_distance.jb1, jb_distance.jb2)
	}

	circuit_sizes := make(map[int]int)
	for i := range n {
		i_root := union_find.find(i)
		circuit_sizes[i_root] += 1
	}

	values := NewSet[int]()
	for _, v := range circuit_sizes {
		values.add(v)
	}
	list_values := values.list_of_elements()

	slices.Sort(list_values)
	slices.Reverse(list_values)

	return list_values[0] * list_values[1] * list_values[2]
}
...
func main() {
	fmt.Println(solution("./example.txt", 10))  // 40
	fmt.Println(solution("./input.txt", 1_000)) // 68112
}
